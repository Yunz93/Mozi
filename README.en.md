...
![Moji preview](https://raw.githubusercontent.com/Yunz93/PicRepo/main/image/M%20%E8%A8%98-1776170252301.png)
...
Signed and notarized GitHub Releases should also open normally after you drag **墨记.app** into Applications. If Gatekeeper still blocks the app, run:

```bash
xattr -cr /Applications/墨记.app
```

If it is still in Downloads, use `~/Downloads/墨记.app` instead.
...
Moji is built on many excellent open-source projects (in no particular order). The tables below list the main runtime dependencies used by the product; full versions and transitive deps live in `package.json` / `package-lock.json` and `src-tauri/Cargo.toml` / `Cargo.lock`.
...
On the Rust side, Moji also uses [serde](https://serde.rs/), [reqwest](https://github.com/seanmonstar/reqwest), [uuid](https://github.com/uuid-rs/uuid), and crypto-related crates (such as `chacha20poly1305`, `hmac`, `sha2`) for secure local storage and desktop networking.
